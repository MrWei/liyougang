...
Class PublicCls
	...
	'发送手机验证码,返回成功发送的条件
	Function SendMobileMsg(Mobile,Content)
   ...
   '---------------------------------------------------------------------------------------------------------
	...
	Public Function GetFileName(FsoType, AddDate, Ext)
		...
		Y=Year(AddDate):M=Right("0"&Month(AddDate),2):D=Right("0"&Day(AddDate),2)
		 Select Case FsoType
		  Case 1:GetFileName = Y & "/" & M & "-" & D & "/" & MakeRandom(N) & Ext  '年/月-日/随机数+扩展名
		  Case 2:GetFileName = Y & "/" & M & "/" & D & "/" & MakeRandom(N) & Ext '年/月/日/随机数+扩展名
		  Case 3:GetFileName = Y & "-" & M & "-" & D & "/" & MakeRandom(N) & Ext '年-月-日/随机数+扩展名
		  Case 4:GetFileName = Y & "/" & M & "/" & MakeRandom(N) & Ext '年/月/随机数+扩展名
		  Case 5:GetFileName = Y & "-" & M & "/" & MakeRandom(N) & Ext '年-月/随机数+扩展名
		  Case 12:GetFileName = Y & M & "/" & MakeRandom(N) & Ext '年-月/随机数+扩展名
		  Case 6:GetFileName = Y & M & D & "/" & MakeRandom(N) & Ext '年月日/随机数+扩展名
		  Case 7:GetFileName = Y & "/" & MakeRandom(N) & Ext '年/随机数+扩展名
		  Case 8:GetFileName = Y & M & D & MakeRandom(N) & Ext '年+月+日+随机数+扩展名
		  Case 9:GetFileName = MakeRandom(N) & Ext
		  Case 10:GetFileName = MakeRandomChar(N) & Ext '随机字符
		  Case 11:GetFileName ="ID"
		  Case Else
		   GetFileName = Y & M & D & Ext '12位随机数+扩展名
		End Select
	End Function
	...
	'获取分页链接
		Function GetPageUrl(ChannelID,ID,CurrPage)
		      Dim Url
		      If C_S(ChannelID,48)=0 or channelid=-1 Then
	...
	OrderDetailStr=OrderDetailStr & "		  </tr> "
		...
		OrderDetailStr=OrderDetailStr & "	   <td width='65' style='text-align:center'>" & formatnumber(rsi("price_original"),2) & "</td>    "
		OrderDetailStr=OrderDetailStr & "	   <td width='65' style='text-align:center'>" & formatnumber(rsi("realprice"),2) & "</td>    "
		
		if flag=1 then
			OrderDetailStr=OrderDetailStr & "	   <td width='65' style='text-align:center'>" & formatnumber(rsi("realprice"),2) 
		...
		if ChkClng(rs("UseScoreisshop"))>0 then
			OrderDetailStr=OrderDetailStr & ""& ChkClng(rs("UseScoreisshop")) &" 积分 + "	& formatnumber(rsi("realprice"),2) 
		else
			OrderDetailStr=OrderDetailStr  & formatnumber(rsi("realprice")*rsi("amount"),2) 		
		end if
		OrderDetailStr=OrderDetailStr  & "</td>    "
		...
		OrderDetailStr=OrderDetailStr & "<b>" & formatnumber(totalprice,2) & "</b></td>    "
	   ...
	   OrderDetailStr=OrderDetailStr & "&nbsp;&nbsp;运费：" & rs("Charge_Deliver")&" 元&nbsp;&nbsp;&nbsp;&nbsp;税率：" & Setting(65) &"%&nbsp;&nbsp;&nbsp;&nbsp;价格含税："
				...
				OrderDetailStr=OrderDetailStr & "</td>"
    OrderDetailStr=OrderDetailStr & "<td  colspan='3' style='text-align:right'><b>订单金额：</b> ￥" & formatnumber(rs("NoUseCouponMoney"),2) & " 元<br>"
	...
	OrderDetailStr=OrderDetailStr & "<b>应付金额：</b> <span style=""font-weight:bold;font-size:15px;color:#ff6600"">￥" & formatnumber(rs("MoneyTotal"),2) & "</span>  元"
	OrderDetailStr=OrderDetailStr &"</td>"
    OrderDetailStr=OrderDetailStr & "<td colspan='3' align='left'><b>已付款：</b> "
	
	OrderDetailStr=OrderDetailStr & "￥<font color=red>" & formatnumber(rs("MoneyReceipt"),2) & "</font></b>"
	
	If RS("MoneyReceipt")<RS("MoneyTotal") Then
	OrderDetailStr=OrderDetailStr & "<br><B>尚欠款：￥<font color=blue>" & formatnumber(RS("MoneyTotal")-RS("MoneyReceipt")+rs("UseCouponMoney"),2) &"</B>"
	End If
...
End Function
  ...
  '======================================会员相关函数====================================
   ...
   Function GetPrice(price)
     If Not IsNumeric(price) Then GetPrice="0": Exit Function
	 If Price=0 Then
	   GetPrice="0"
	 ElseIf Price<1 Then
	   GetPrice="0"&Price
