...
Class Refresh
		...
		'返回评论
		Function GetCommentList(CurrentPage,PostTable,postid,ModelID,ItemID,Tid,Title,Fname,cmtnum,totalPut,MaxPerPage,PageNum)
				...
				CommentStr=Replace(CommentStr,"{$ItemUrl}",KS.GetItemUrl(ModelID,Tid,itemid,Fname))
				Else
			...
			case "historygroupbuylink" If KS.ChkClng(KS.Setting(179))=1 Then Echo DomainStr & "groupbuy/history/" Else Echo DomainStr & "shop/groupbuy.asp?flag=history"
			 ...
			 case "getshowcomment" 
			   If GetNodeText("comment")="1" and KS.GetAppStatus("digmood") Then 
				...
				echo GetCommentList(1,GetNodeText("posttable"),KS.ChkClng(GetNodeText("postid")),ModelID,ItemID,GetNodeText("tid"),GetNodeText("title"),GetNodeText("fname"),GetNodeText("cmtnum"),totalPut,MaxPerPage,PageNum)
			   ...
			   If BindChannelID>0 and BindInfoID>0 Then
				  ...
				  If Not RSB.Eof Then
				    if GFieldName="url" then
					 echo KS.GetItemURL(BindChannelID,RSB("Tid"),BindInfoID,RSB("Fname"))
					Else
				    echo RSB(GFieldName)
					End If
				  End If
						...
						If IsArray(SQL) Then
						  For I=0 To Ubound(SQL,2)
							...
							If I=0 Then
							 ...
							 'GroupImgList="<li><img class=""curr"" width=""40"" title=""" &sql(4,i) & """ src=""" & Spic &""" alt=""" & Bpic & """ /></li>"
							Else
							 ...
							 'GroupImgList=GroupImgList & "<li><img width=""40"" title=""" &sql(4,i) & """ src=""" & Spic &""" alt=""" & Bpic & """ /></li>"
							End If
						  Next
						 Else
	   ...
	   End Function
		...
		Function LoadTemplate(TemplateFname)
		    If KS.Setting(187)="0" Then
		      ...
		      KS.Die "<div style='margin:0px auto;width:700px;border:1px solid #f9c943;background:#FFFFF6;padding:10px;margin-top:120px;'>" & WebTips & "</div>"
			End If
							...
							For I = 0 To Tpage - 1
								...
								if CurrPage<=1 then  n=0 else n=pagenum*(CurrPage-1)
								   For r=1 to pagenum
										  if n<=ubound(ContentArr) Then	
										   BigPic=Split(ContentArr(n),"|")(1) : If lcase(Left(BigPic,4))<>"http" Then BigPic=KS.Setting(2) & BigPic
							               ThumbList=ThumbList & "<div><img title=""" &Split(ContentArr(n), "|")(0) & """ class=""scrollLoading"" onclick=""javascript:window.open(this.src)"" alt='" & Split(ContentArr(n), "|")(0)& "' style=""cursor:hand;background:url(" & DomainStr &"images/default/loading.gif) no-repeat center;""  data-url=""" &BigPic &""" src=""" & DomainStr &"images/default/pixel.gif"" border='0'><br/><span>" &Split(ContentArr(n), "|")(0) & "</span></div>"
										  Else 
										   Exit For
										  End If
										  n=n+1
