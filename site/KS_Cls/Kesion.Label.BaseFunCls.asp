...
Class LabelBaseFunCls
		...
		'后台修改KS_Article表数据
		Sub UpdateArticle(ChannelID,InfoID,Title,Tid,Intro,KeyWords,PhotoUrl,AddDate,Hits,HitsByDay,HitsByWeek,HitsByMonth,Recommend,Rolls,Strip,Popular,Slide,IsTop,Comment,Verific)
		 Gcls.Execute("Update [KS_ItemInfo] Set Title='" & replace(Title,"'","''") & "',Tid='" & Tid & "',Intro='" & Replace(left(KS.LoseHtml(Intro),255),"'","''")  & "',KeyWords='" & Replace(KeyWords,"'","''") & "',PhotoUrl='" & PhotoUrl & "',AddDate='" & AddDate & "',ModifyDate=" & SQLNowString & ",Hits=" & KS.ChkClng(Hits) & ",HitsByDay=" & KS.ChkClng(HitsByDay) & ",HitsByWeek=" & KS.ChkClng(HitsByWeek) & ",HitsByMonth=" & KS.ChkClng(HitsByMonth) & ",Recommend=" & KS.ChkClng(Recommend) & ",Rolls=" & KS.ChkClng(Rolls) & ",Strip=" & KS.CHkClng(Strip) & ",Popular=" & KS.ChkClng(Popular) & ",Slide=" & KS.ChkClng(Slide) & ",IsTop=" & KS.ChkClng(IsTop)  &",Comment=" & KS.ChkClng(Comment) & ",Verific=" & KS.CHkClng(Verific) & ",Otid='" & KS.G("Otid") &"',oid=" & KS.ChkClng(KS.S("Oid")) &" Where  ChannelID=" & KS.ChkClng(ChannelID) & " and InfoID=" & KS.ChkClng(InfoID))
		End Sub		
