...
Class ManageCls
       ...
       '取得专题
		Sub Get_KS_Admin_Special(ChannelID,InfoID)
		   With KS
		     .echo "<dd><div>选择专题:<font>(<a href='javascript:;' onclick=""$('#SpecialID option').each(function(){$(this).attr('selected',false);});"">取消选定的专题</a>)</font></div>"
					   ...
					   Select Case fieldtype
						 ...
						 Case 10
							...
							If GetEditorType()="CKEditor" Then
							    GetDiyField=GetDiyField & "<textarea id=""" & fieldname &""" name=""" & fieldname &""">"& Server.HTMLEncode(F_Value) &"</textarea><script type=""text/javascript"">CKEDITOR.replace('" & fieldname &"', {width:""" & XWidth &""",height:""" & Xheight & """,toolbar:""" & Node.SelectSingleNode("editortype").text & """,filebrowserWindowWidth:650,filebrowserWindowHeight:290});</script>"
							Else	
								...
								if XMaxlength<>0 then
								 GetDiyField=GetDiyField & "<script>setTimeout(""" & fieldname &" = " & GetEditorTag() &".getEditor('" & fieldname &"',{toolbars:[" & GetEditorToolBar(Node.SelectSingleNode("editortype").text) &"],maximumWords:" &XMaxlength & "});"",10);</script>"
								Else
								 GetDiyField=GetDiyField & "<script>setTimeout(""" & fieldname &" = " & GetEditorTag() &".getEditor('" & fieldname &"',{toolbars:[" & GetEditorToolBar(Node.SelectSingleNode("editortype").text) &"],wordCount:false});"",10);</script>"
								End If
							End If
