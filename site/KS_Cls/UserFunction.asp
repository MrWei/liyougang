...
Function Parse(sTemplate, iPosBegin)
 ...
 <input name="Verifycode" id="Verifycode" type="text" class="yzmInput" maxlength="6" size="10" autocomplete="off" />
  <span id="showVerify"><img style='margin-top:10px;height:28px;cursor:pointer' title='点击刷新' align='absmiddle' src='../../plus/verifycode.asp' onClick='this.src="../../plus/verifycode.asp?n="+ Math.random();'></span>
				</dd>
		<% End If
		case "showstyle"
		   if action="Edit" Then
     ...
     %>
		...
		<script type="text/javascript">
			function  fqiandao(){
				 ...
				 $.get("User_qiandao.asp",{qdxq:qdxq,qdContent:qdContent,action:"qiandao",anticache:Math.floor(Math.random()*1000)},function(d){//读取列表
		...
		<%
		if not conn.execute("select username from KS_qiandao where username='" & ksuser.username &"' and year(AddDate)=year(" & SqlNowString & ") and month(AddDate)=month(" & SqlNowString &") and day(AddDate)=day(" & SqlNowString & ") ").eof then
		...
		<div class="left02">
         ...
         if KSUser.CheckPower("s16")=true then
		    Response.Write "<li onMouseOver=""this.className='hvr'"" onMouseOut=""this.className=''"">"
			Response.Write"	<img src=""images/star.gif"" align=""absmiddle"" /> <a href=""User_favorite.asp"">我的收藏</a>"
		 ...
		 If KSUser.CheckPower("s19")=true Then 
		    Response.Write "<li onMouseOver=""this.className='hvr'"" onMouseOut=""this.className=''"">"
			Response.Write "<img src=""images/icon1.png"" align=""absmiddle"" /> <a href=""user_form.asp"">表单管理</a>"
			Response.Write "</li>"
		 End If
	   ...
	   End If
		 ...
		 '团购
		  If KSUser.CheckPower("s09")=true  and   KS.C_S(5,21)="1" Then 
		    Response.Write "<li onMouseOver=""this.className='hvr'"" onMouseOut=""this.className=''"">"
			Response.Write"	<img src=""images/icon9.png"" align=""absmiddle"" /> <a href=""User_groupbuy.asp"">团购</a>"
		...
		If KS.C_S(10,21)=1 Then
			If KSUser.GetUserInfo("UserType")=0 Then
				If KSUser.CheckPower("s14")=true Then 
					 If KS.C_S(10,21)="1" Then
						Response.Write "<li onMouseOver=""this.className='hvr'"" onMouseOut=""this.className=''"">"
						Response.Write "<img src=""images/icon10.png"" align=""absmiddle"" /> <a href=""User_JobResume.asp"">求职</a>"
			...
			if KSUser.CheckPower("s14")=true  then
						Response.Write "<li onMouseOver=""this.className='hvr'"" onMouseOut=""this.className=''"">"
						Response.Write "<img src=""images/icon10.png"" align=""absmiddle"" /> <a href=""user_Enterprise.asp?action=job"">招聘</a>"
...
Function GetInputForm(IsTemplate,ChannelID,FieldXML,FieldNode,FieldDictionary,id,KSUser,RS)
  Dim FNode,ClassID,Title,KeyWords,Author,Origin,Content,Verific,PhotoUrl,BigPhoto,Intro,FullTitle,ReadPoint,Province,City,UserDefineFieldArr,I,SelButton,MapMarker,PicUrls,ShowStyle,PageNum,DownSize,SizeUnit,DownPT,YSDZ,ZCDZ,JYMM,DownUrls,FlashUrl,ChargeTips,AddDate,oTid,oID,SeloTid,ChangesUrl,Changes
if IsObject(RS) And IsTemplate=false Then
	If Not RS.Eof Then
		...
		Else
		 ...
		 Else
		    ShowStyle="[#ShowStyle]":PageNum="[#PageNum]":PicUrls="[#PicUrls]":Title="[#Title]":FullTitle="[#FullTitle]":KeyWords="[#KeyWords]":Author="[#Author]":Origin="[#Origin]":Province="[#Province]":City="[#City]":Author="[#Author]":Intro="[#Intro]":Content="[#Content]":PhotoUrl="[#PhotoUrl]":BigPhoto="[#BigPhoto]":ReadPoint="[#ReadPoint]":Verific="[#Verific]":MapMarker="[#MapMarker]":DownSize="[#DownSize]":DownPT="[#DownPT]":YSDZ="[#YSDZ]":ZCDZ="[#ZCDZ]":JYMM="[#JYMM]":DownUrls="[#DownUrls]":AddDate="[#AddDate]":ChangesUrl="[#ChangesUrl]"
			'自定义字段
...
<dd>
     ...
     <input class='textbox' type='text' size=70 style="width:<%=XWidth%>px" name='DownPT' value="<%=DownPT%>"><br>
		 <font color='#808080'>平台选择 <%If Istemplate Then%>[#DownPTList]<%Else%><%=DownPTList%><%End If%></font>
</dd>
<%case "ysdz"%>
<dd>
   <div><%=XTitle%>：</div>
...
<dd>
    <div><%=XTitle%>：</div>
    <script>try{setCookie("pid",'<%=province%>');}catch(e){}</script>
...
<dd ID='ContentArea'>
  ...
  <%
	...
	%>
	 ...
	 <%end if%>
		<%
			 Response.Write "<script id=""Content"" name=""Content"" type=""text/plain"" style=""width:90%;height:420px;"">" & Content &"</script>"
	         Response.Write "<script>setTimeout(""editor = " & GetEditorTag() &".getEditor('Content',{toolbars:[" & Replace(GetEditorToolBar("NewsTool")," 'source', '|',","") &"],wordCount:false,autoHeightEnabled:false,minFrameHeight:420,elementPathEnabled:false });"",10);</script>"
