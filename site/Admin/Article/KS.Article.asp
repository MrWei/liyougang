...
Class Admin_Article
		...
		Private NewsID, TitleType, Title,Fulltitle,ShowComment, TitleFontColor, TitleFontType, PicNews, ArticleContent, PhotoUrl, Changes, Recommend,IsTop,PageTitle,IsSign,SignUser,SignDateLimit,SignDateEnd,Province,City,RelatedID,Otid,OID
	 ...
	 Sub ArticleManage()
			...
			ElseIf Action = "Edit"  Or Action="Verify" Then
			   ...
			   End If
				...
				if ChannelID=1 then RelatedID      = RS("RelatedID")
  ...
  For Each TypeNode In Application(KS.SiteSN & "_FieldGroupXml").DocumentElement.SelectNodes("row[@channelid=" & ChannelID &"]")
	...
	For Each FNode In FieldXML.DocumentElement.SelectNodes("fielditem[showonform=1&&fieldtype!=13&&@groupid=" & TypeNode.SelectSingleNode("@id").text & "]")
		...
		Else
	     ...
	     Select Case lcase(FNode.SelectSingleNode("@fieldname").text)
	       case "title"
				  ...
				  IF Action="Verify" and  Inputer<>KS.C("AdminName") and KS.C("Role")="3" Then
				   ...
				   End If
					...
					End If
			        .Write "<input class='button' type='button' value='重名检测' onclick=""if($('#title').val()==''){ top.$.dialog.alert('请输入" & KS.C_S(ChannelID,3) & "标题!');}else top.openWin('文章重名检测','article/KS.Article.asp?ChannelID=" & ChannelID & "&Action=CheckTitle&title='+escape($('#title').val()),false,360,370);"">"
					...
					if RelatedID=-11 or KS.ChkClng(RelatedID)<>0 then
						.Write "<div style=""padding:5px;margin-top:3px;border:1px solid #f9c943;background:#FFFFF6""><label><input type='checkbox' name='EditNewtb' value='1' checked/> 此"  & KS.C_S(ChannelID,3) & "同时发布到多个栏目，选中将同步关联" & KS.C_S(ChannelID,3) & " <input type='hidden' name='RelatedID' value='"& RelatedID &"'/></label></div>"
					end if
					End If
		  ...
		  case "tid"
					...
					.Write " <div style=""position:relative;float:left;font-weight:normal""><input type='hidden' name='OldClassID' value='" & FolderID & "'>"
					.Write " <select size='1' name='tid' id='tid' style='width:335px'>"
					.Write " <option value='0'>--请选择" & KS.GetClassName(ChannelID) &"--</option>"
					.Write Replace(KS.LoadClassOption(ChannelID,true),"value='" & FolderID & "'","value='" & FolderID &"' selected") & " </select></div>"
					...
					var box=''
					function sel(){
					top.openWin(false,'article/KS.Article.asp?channelID=<%=ChannelID%>&FolderID='+$("#tidtb").val()+'&action=SelectClass',false,360,360);
					}
					</script>
					<%
		...
		case "intro"
				...
				.Write "  <textarea class='textbox' name=""Intro"" style='width:95%;height:80px'>" & Intro & "</textarea>"
				.Write " </dd>" &vbcrlf
		case "content"
			    ...
			    .Write EchoEditor("Content",ArticleContent,"NewsTool","96%","350px")
		...
		case "photourl"
				...
				If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='photourl']/showonform").text="1" Then
				   .Write "<div style=""float:right;margin:0 auto;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=scale);height:40px;width:45px;border:1px solid #ccc""><img src=""" & PhotoUrl & """ onerror=""this.src='../images/nopic.gif';"" id=""pic"" style=""height:40px;width:45px;""></div>"
				end if
...
END IF
		...
		If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='attroption']/showonform").text="1" Then
	      ...
	      If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='adddate']/showonform").text="1" Then
			...
			If Action <> "Edit" Then
			.Write ("<input name='AddDate' type='text' onclick=""WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"" id='AddDate' value='" & Now() & "' size='50'  class='textbox'>")
			Else
			.Write ("<input name='AddDate' type='text' onclick=""WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"" id='AddDate' value='" & AddDate & "' size='50'  readonly class='textbox'>")
			End If
			.Write " <a href='#' onclick=""WdatePicker({el:$('input[name=AddDate]').get(0),dateFmt:'yyyy-MM-dd HH:mm:ss'});return false;""><img src='../Images/date.gif'  border='0' align='absmiddle' title='选择日期'></a> <b>日期格式：年-月-日 时：分：秒"
			.Write "               </dd>"
	    End If
	  ...
	  If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='template']/showonform").text="1" Then
			...
			.Write "</div>"
			Else
			.Write "<div style='font-weight:normal' id='templatearea'>"
			If KS.WSetting(0)="1" Then .Write "<strong>WEB模板</strong> "
			.Write "<input id='TemplateID' name='TemplateID' readonly maxlength='255' size=50 class='textbox' value='" & TemplateID & "'>&nbsp;" & KSCls.Get_KS_T_C("$('#TemplateID')[0]")
			If KS.WSetting(0)="1" Then 
			.Write "<br/><strong>3G模板</strong> "
			.Write "<input id='WapTemplateID' name='WapTemplateID' readonly size=50 class='textbox' value='" & WapTemplateID & "'>&nbsp;" & KSCls.Get_KS_T_C("$('#WapTemplateID')[0]") 
			End If
			.Write "</div>"
			End If
	  ...
	  If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='fname']/showonform").text="1" Then
			...
			.Write "<div id='filearea' style='display:none;font-weight:normal'><input name='FileName' type='text' id='FileName'   value='" & FileName  & "' size='25' class='textbox'> <font color=red>可带路径,如 help.html,news/news_1.shtml等</font></div>"
			End IF
			 .Write "             </dd>"
     End If
	 		.Write "              <dd>"
	   ...
	   If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='signoption']/showonform").text="1" Then
			   ...
			   .Write "	</SCRIPT>"
				...
				.Write "           <input type='text' class=""textbox"" onclick=""WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"" id='SignDateEnd'  name='SignDateEnd' value='" & SignDateEnd & "'> <a href='#' onclick=""WdatePicker({el:$('input[name=SignDateEnd]').get(0),dateFmt:'yyyy-MM-dd HH:mm:ss'});return false;"" ><img src='../Images/date.gif'  border='0' align='absmiddle' title='选择日期'></a> <font color=blue>签收用户必须在这个时间内完成签收。格式：期格式：年-月-日 时：分：秒</font></dd>"
				.Write "        </span>"
		...
		Sub DoSave()
				...
				If Not KS.IsNul(PhotoUrl) Then PicNews=1 Else PicNews=0
					...
					if KS.ChkClng(KS.G("EditNewtb"))=1 then
						if KS.ChkClng(KS.G("RelatedID"))=0 or KS.ChkClng(KS.G("RelatedID"))=-11 then
							RelatedArray=GetRelatedArray( KS.C_S(ChannelID,2), NewsID ,11)'同步文章
						else
							RelatedArray=GetRelatedArray( KS.C_S(ChannelID,2), KS.ChkClng(KS.G("RelatedID")) ,22)'同步文章
						end if 
					else
			...
			Function GetRelatedArray(ChannelID,Related_ID,lx)
	...
	Sub SelectClass()
	   ...
	   response.cachecontrol="no-cache"
		...
		With Response
			...
			.Write "<link href='../Include/Admin_Style.CSS' rel='stylesheet' type='text/css'>"
			.Write "<body style=""padding:10px;text-align:center"">"
						.Write "<br/><select size='1' name='tidtb' id='tidtb' multiple=""multiple"" style=""width:300px; height:250px;"">"
