...
<tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	<a href="?action=modify&id=<%=rs("id")%>" onclick="window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户管理 >> <font color=red>修改圈子信息</font>')+'&ButtonSymbol=GOSave';">编辑</a>
	
	<a href="?Action=Del&ID=<%=rs("id")%>" onclick="return(confirm('删除圈子将删除圈子下的所有信息，确定删除吗？'));">删除</a> &nbsp;<%if rs("verific")=0 then%><a href="?Action=verific&id=<%=rs("id")%>">审核</a> <%elseif rs("verific")=1 then%><a href="?Action=lock&id=<%=rs("id")%>">锁定</a><%elseif rs("verific")=2 then%><a href="?Action=unlock&id=<%=rs("id")%>">解锁</a><%end if%>
...
Dim TeamName,username,PhotoUrl,ClassID,Announce,Note,Verific,ViewTF,JoinTF,Recommend
 ...
 <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" class="ctable">
          ...
          <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
           ...
           <td height='28'>&nbsp; <%
				 Response.Write "<script id=""Note"" name=""Note"" type=""text/plain"" style=""width:80%;height:200px;"">" &Note&"</script>"
	             Response.Write "<script>setTimeout(""var editor = " & GetEditorTag() &".getEditor('Note',{toolbars:[" & GetEditorToolBar("basic") &"],wordCount:false,autoHeightEnabled:false });"",10);</script>"
			%> </td>
          </tr>  
          <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>空间公告：</strong></td>
           <td height='28'>&nbsp; <%
				 Response.Write "<script id=""Announce"" name=""Announce"" type=""text/plain"" style=""width:80%;height:200px;"">" &Announce&"</script>"
	             Response.Write "<script>setTimeout(""var editor = " & GetEditorTag() &".getEditor('Announce',{toolbars:[" & GetEditorToolBar("basic") &"],wordCount:false,autoHeightEnabled:false });"",10);</script>"
			%>
		   </td>
          </tr>  
          <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
