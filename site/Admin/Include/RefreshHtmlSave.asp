...
Class RefreshHtmlSave
		...
		Public Sub Kesion()
			...
			Select Case Types
			 Case "Content"
			            If KS.C_S(ChannelID,7)<>1 and KS.C_S(ChannelID,7)<>2 Then Call KS.AlertHistory("KesionCMS系统提醒您：<br/><br/>1、此模型内容页没有启用生成静态HTML功能<br/><br/>、请到模型管理->模型信息设置启用生成静态Html功能",-1):Exit Sub
						Call RefreshContent
			 Case "Folder"
			          
			          If ChannelID<>0 and KS.C_S(ChannelID,7)<>1 Then Call KS.AlertHistory("KesionCMS系统提醒您：<br/><br/>1、此模型栏目页没有启用生成静态HTML功能<br/><br/>、请到模型管理->模型信息设置启用生成静态Html功能",-1):Exit Sub
...
'                                                     以下为本模块相应处理的函数		'================================================================================================================================
				...
				If PauseNum>0 Then
					If RefreshTotalNum>1 and NowNum Mod PauseNum=0 Then
					    R_RS.Close:Set R_RS=Nothing
						 ...
						 .echo "</script>" &vbcrlf
						 .die "<meta http-equiv=""refresh"" content=""" & PauseTime & ";url=RefreshHtmlSave.asp?f=" & f & "&CurrNowNum=" & NowNum+1 & "&ChannelID=" & ChannelID & "&RefreshFlag=Pause&Types=" & Types & "&StartRefreshTime=" & StartRefreshTime &  "&RefreshTotalNum=" & RefreshTotalNum & """>"
					End If
			   End If
			Next
		...
		Function GetRefreshSucc(Node,ItemName)
		 ...
		 if Types="Content" and Not KS.IsNul(KS.Setting(189)) Then
		     dim baiduts
