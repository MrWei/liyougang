	...
	Sub EditLabelSave()
			...
			Else
				...
				LabelRS.Open "Select top 1 * From [KS_Label] Where ID='" & LabelID & "'", Conn, 1, 3
				 ...
				 If KeyWord = "" Then
				   	KS.Echo "<script>top.$.dialog.alert('<br/>恭喜，标签修改成功!',function(){$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & FolderID & "&OpStr=" & server.URLEncode("标签管理  >> 自定义函数标签") &"&ButtonSymbol=DIYFunctionLabel';location.href='include/Label_main.asp?Page=" & Page & "&LabelType=5&FolderID=" & FolderID & "';});</script>"

				 Else
				   	KS.Echo "<script>top.$.dialog.alert('恭喜，标签修改成功!',function(){$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?OpStr=" & server.URLEncode("标签管理 >> <font color=red>搜索自定义函数标签结果</font>") &"&ButtonSymbol=DIYFunctionSearch';location.href='include/Label_main.asp?Page=" & Page & "&LabelType=5&KeyWord=" & KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate & "&EndDate=" & EndDate & "';});</script>"
				 End If
			End If
	End Sub
