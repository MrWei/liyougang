       ...
       Sub Kesion()
		...
		function EditGroup(ID){
		 ...
		 $(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape("用户组管理 >> <font color=red>修改用户组</font>")+'&ButtonSymbol=GoSave';
			...
			Response.Write "<div id='mtl'>用户组管理导航：</div><li><a href=""KS.UserGroup.asp"">管理首页</a>&nbsp;|&nbsp;<a href=""#"" onclick=""AddGroup(0)"">新增用户组</a>"
	...
	%>
		  <tr height="30" align="center" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'"> 
           ...
           <input type='checkbox' name='id' id='c<%=rs("ID")%>' value='<%=rs("ID")%>'<%if rs("Type")<>"1" then response.write " disabled"%>>
			...
			<td class="splittd" width="390" style="text-align:left"><%=rs("Descript")%> </td>
			...
			<td class="splittd" width="200">
			<a href="javascript:AddGroup(<%=RS("ID")%>)">添加子用户组</a>
			...
			%>
			<a href="KS.User.asp?UserSearch=10&GroupID=<%=RS("ID")%>">列出会员</a>
		...
		<%
		KS.echo "<tr><td height='50' style='padding-left:20px' align='right' colspan=4><div style='margin:5px'><b>选择：</b><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a> <input type='Submit' onclick=""return(confirm('删除用户组操作将删除此用户组的所有子用户组，并且不能恢复！确定要删除选中的用户组吗?'))"" class='button' value='删除选中的用户组'></div>"
		KS.echo "</form>"
	    KS.echo "</td></tr>"
		...
		<table class='ctable' width="100%" height=273 border=0 align="center" cellpadding=1 cellspacing=1  style='margin:1px'>
			<tr class="tdbg"> 
			  ...
			  If frmAction="Modify" Then
				...
				Response.Write "<select name='parentID1' Disabled>" & vbCrLf
				Else
...
<input type="text" name="SpaceSize" size="10" class="textbox" style="text-align:center" value="<%=SpaceSize%>" />KB <font color="#FF0000">提示：1 KB = 1024 Byte，1 MB = 1024 KB</font> </td>
			...
			<tr class="tdbg">
			  ...
			  <td height="30">
				...
				发布信息时获得积分为模型设置的<input type='text' class='textbox' name='GroupSetting3' value="<%=GroupSetArr(3)%>" style='text-align:center;width:30px' />倍 <font color='blue'>输入"0" 表示此会员组不得分</font><br/>
				发布信息时获得点券为模型设置的<input type='text' class='textbox' name='GroupSetting4' value="<%=GroupSetArr(4)%>" style='text-align:center;width:30px' />倍 <font color='blue'>输入"0" 表示此会员组不得点券</font><br/>
				发布信息时获得资金为模型设置的<input type='text' class='textbox' name='GroupSetting5' value="<%=GroupSetArr(5)%>" style='text-align:center;width:30px' />倍 <font color='blue'>输入"0" 表示此会员组不得资金</font><br/>
...
<td>
				 ...
				 <table border="0" width="100%" cellpadding="0" cellspacing="0">
				   <tr>
					...
					<td> 一天最多发<input type="text" class='textbox' name="GroupSetting27" value="<%=GroupSetArr(27)%>" style="text-align:center;width:40px" />帖子<span class='tips'>如果不想限制请输入“0”</span><br/>
