...
Class Admin_UserMessage
		...
		Public Sub Kesion()
			...
			Response.Write"<div class=""topdashed sort menu_top_fixed"" style=""text-align:left"">用户短信管理：<a href='?action=new'>发送站内短信</a> <a href='?action=sms'>发送手机短信</a></div>"
		...
		Sub ShowContent()
		 ...
		 %>
		  <tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'" id='u<%=RSObj("ID")%>' onclick="chk_iddiv('<%=RSObj("ID")%>')">
			...
			<td class="splittd" align="center"><a href="?action=edit&id=<%=rsobj("id")%>">修改</a> | <a onclick="return(confirm('删除后不可恢复，确定删除吗?'))" href="?action=del&id=<%=rsobj("id")%>">删除</a></td>
		  </tr>
		...
		Sub SaveSendSms_1()
			...
			End If
			ToUserName="'" & Replace(ToUserName,",","','") & "'"
			Call ToSend("select UserName,RealName,Mobile From KS_User Where Mobile In(" & ToUserName &") order by userid")
		End Sub
		'按指定用户组及条件发送
		Sub SaveSendSms_2()
			Dim GroupID,ErrMsg,i
