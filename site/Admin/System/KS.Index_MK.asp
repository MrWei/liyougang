...
Class Main
				 ...
				 %>
				  <script type="text/javascript">
					$(document).ready(function(){
						...
						str=str+("				<img id=\"img_<%=ItemID%>\" src=\"<% =Node.SelectSingleNode("Fastico").text %>\"/> <br/>");
						str=str+("				<a id=\"url_<%=ItemID%>\" href=\"javascript:void(0)\"   onclick=\"SelectObjItem1(this,\'<%=Node.SelectSingleNode("Fast").text %> >> <font color=red><%=Node.SelectSingleNode("Fast").text%></font>\',\'<%=Node.SelectSingleNode("Attribute").text%>\',\'<%=Replace(Node.SelectSingleNode("Fasturl").text,"|Fast|","&")%>\')\" ><%=Node.SelectSingleNode("Fast").text %></a>");
			...
			if  Request("tya")="addmka" then
				 ...
				 If Not Node Is Nothing Then
				     if not FieldXML.DocumentElement.SelectSingleNode("/field/item[Fast='" & Node.SelectSingleNode("Fast").text &"']") is nothing then
					   KS.AlertHintScript "您选择的项目已添加过了！"
					 end if
		...
		Sub MainList()
		   ...
		   if Fastico="" then Fastico="images/mk_1.png"
			...
			%>
			<ul id='menu_top' style='font-weight:bold;text-align:center;padding-top:14px' >
			...
			<style>
			.mk_list{ padding:10px 0px 10px 10px;}
			.mk_list .cp_box{ background:#FFFFFF; padding-top:5px; margin-top:5px;border:1px solid #CCCCCC; float:left; margin-right:15px; width:86px; height:90px; font-size:13px; font-weight:500; text-align:center; position:relative;}
			
			.mk_list .cp_box .but{background:url(../Images/but_delmkb.png); width:18px; height:18px;cursor:pointer;border:none;}
			.mk_list .cp_box .but1{background:url(../Images/but_addmkb.png);width:70px; height:25px;cursor:pointer ;margin-left:8px; border:none; }
			</style>
			
			<div id="mkselsec" style="display:none">
			<form name='myform' method='Post' action='KS.index_mk.asp'>
			<table style=" border:1px solid #CCCCCC; margin-top:10px; " width="600"  border="0" align="center" cellpadding="0" cellspacing="0" >
			...
			<tr>
			<td width="30%" height="50" bgcolor="#E6F3FB" align="center" style="border-bottom:1px solid #CCCCCC" > 快捷名称:</td>
			<td width="70%" bgcolor="#FFFFFF" style="border-bottom:1px solid #CCCCCC; padding-left:5px"><input class="textbox" name="Fast_name" type="text" value="<%=Fast%>" size="10"  /> </td>
			</tr>
			<tr>
			<td width="30%" height="50" bgcolor="#E6F3FB" align="center" style="border-bottom:1px solid #CCCCCC" > 快捷功能地址:</td>
			<td width="70%" bgcolor="#FFFFFF" style="border-bottom:1px solid #CCCCCC; padding-left:5px">
			...
			<tr>
			<td width="30%" height="50" bgcolor="#E6F3FB" align="center"  style="border-bottom:1px solid #CCCCCC"> 图标选择:<br/><font color="#FF0000">图标40X40</font></td>
			<td width="70%" bgcolor="#FFFFFF" style="border-bottom:1px solid #CCCCCC; padding-left:5px">
			<input name="ico_url" type="text"  class="textbox" value="<%=Fastico%>" size="30" />
			<img id="img_ico" src="../<%=Fastico%>" />
			<input name="icobut" value="选择图标.." onclick="$('#Fastico').toggle(200);" type="button"  />
				<div id="Fastico" style="display:none; padding:5px; overflow:hidden;" >
				
				<%for i=1 to 11 %>
				
				<a href="#" style="display:block;float:left;text-align:center;margin:2px;border:1px solid #CCCCCC; width:45px; height:45px;" onclick="$('input[name=ico_url]').val('images/mk_<%=i%>.png');$('#img_ico').attr('src','images/mk_<%=i%>.png');$('#Fastico').toggle(200);" ><img src="../images/mk_<%=i%>.png" style="border:none" width="40" height="40" /></a>
				<%next%>
				</div>
			...
			<td width="30%" height="30" bgcolor="#E6F3FB" align="center" style="border-bottom:1px solid #CCCCCC; display:none"> 序号排列:</td>
			...
			<input name="" value="添加" type="submit" onClick="return(CheckForm())"  style="border:solid 1px #CCCCCC; width:70px; height:35px; font-size:14px; font-weight:900; background:#FFFFFF; cursor:pointer;"  />
			<%else%>
			<input name="" value="保存" type="submit" onClick="return(CheckForm())"  style="border:solid 1px #CCCCCC; width:70px; height:35px; font-size:14px; font-weight:900; background:#FFFFFF; cursor:pointer;"  />
			<%end if%>
			<%if Request("Action")="add" then%>
			<input name="" value="返回"  type="button" style="border:solid 1px #CCCCCC; width:70px; height:35px; font-size:14px; font-weight:900; background:#FFFFFF; cursor:pointer;"  onclick="$('#mkselsec').toggle(200);"  />
			<%else%>
			<input name="" value="返回"  type="button" style="border:solid 1px #CCCCCC; width:70px; height:35px; font-size:14px; font-weight:900; background:#FFFFFF; cursor:pointer;"  onclick="$('#mkselsec').toggle(200);"  />
			<%end if%>
			</div>
			...
			<%
			Response.Write "<div id=""cpbox""  class=""mk_list"" >"
			dim Nodekall
			...
			<img id="img_<%=Nodekall.SelectSingleNode("@id").text%>" src="../<% =Nodekall.SelectSingleNode("Fastico").text %>"/> <br/>
			 ...
			 <input name="" class="but1"  value="" type="button" onclick="add_mk_all('<%=Nodekall.SelectSingleNode("@id").text%>');" /> 
			<%	
			Response.Write "</div>"
			Next
