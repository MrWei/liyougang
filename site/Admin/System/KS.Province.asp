...
<script src="../../KS_Inc/common.js"></script>
			<script language="javascript">
				...
				var box='';
				function AreaAdd(){
				 box=top.$.dialog.open('system/KS.Province.asp?Action=add&parentid=<%=ks.s("parentid")%>',{title:'新增地区',width:630,height:200});
				}
				function EditArea(id){
					 box=top.$.dialog.open('system/KS.Province.asp?Action=add&ID='+id,{title:'编辑地区',width:630,height:200});
				}
				function DelArea(id){
...
<li class='parent' onClick="top.frames['MainFrame'].box.close();"><span class=child onMouseOver="this.parentNode.className='parent_border'" onMouseOut="this.parentNode.className='parent'"><img src='../images/ico/back.gif' border='0' align='absmiddle'>取消返回</span></li>
</ul>
...
Sub Add_Submit_Save()
 ...
 If Id=0 Then
 Response.write "<script>top.$.dialog.confirm('添加成功,继续添加吗?',function(){ location.href='system/KS.Province.asp?action=add&parentid=" & parentid & "';},function(){top.frames[""MainFrame""].location.reload();top.frames[""MainFrame""].box.close();})</script>"
 ELse
