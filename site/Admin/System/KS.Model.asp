...
Class Admin_Model
		...
		if request("flag")<>"menu" then
			...
			End IF
			.Write "<li class='parent' onclick=""location.href='KS.Model.asp?action=total';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/set.gif' border='0' align='absmiddle'>数据统计</span></li>"
			.Write "<li class='parent' onclick=""location.href='KS.Model.asp?action=Order';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/order.gif' border='0' align='absmiddle'>模型排序</span></li>"
			.Write "</ul>"
		end if
		  ...
		  With Response
				...
				.Write " </tr>"
				  ...
				  MaxPerPage=50
					...
					End If
				 .Write "<tr><td colspan='2' height=""50"">&nbsp;&nbsp; <input type='submit' class='button' value='批量保存排序'> </td></form>"
...
<strong>特别提醒：</strong><br/>
	...
	<li>搜索的值:即用于供数据库搜索的值,搜索值用英文逗号分开      不能带单位,如 1-20,20-30</li>
		...
		Sub ManageMenu()
		 ...
		 <input type="hidden" name="from" value="<%=from%>"/><br/>
		 <table width='100%' border='0' cellspacing='0' cellpadding='0'> 
		 <%if request("flag")<>"menu" then%> 
		 <tr>
		...
		<%end if%>
		 ...
		 <td>
           <style>
		    .list{}
			.list li{width:25%;float:left;height:25px;}
		   </style>
          <div class="list">
          <ul>
          <%
		  ...
		  case 8
			...
			For Fi=0 To Ubound(Arr)
			  response.write "<li><label><input type=""checkbox"" value=""" &arr(Fi) & """ name=""hasfield"""
			  if instr(lcase(XmlFields),lcase(arr(Fi)))>0 then response.write " checked"
			  response.write ">"& arr(Fi) &"</label></li>"
		 ...
		 <tr class='tdbg'>
		   <td colspan=4 style="height:50px;padding-left:30px;">
...
管理列表显示的字段越少则查询显示速度会越快,一般不常用的字段建议不要选择。
		...
		<div class="Pluslist">
        <div class="tit">系统模型 <a href="javascript:;" onclick="location.href='?action=Add';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Go&OpStr=<%=Server.URLEncode("模型管理 >> <font color=red>添加模型</font>")%>';">添加模型</a></div>
        <div class="co">
        <ul>
        <%
		Dim RS:Set RS=Server.CreateObject("ADODB.RECORDSET")
		...
		Do While Not RS.Eof
		 ...
		 If KS.CheckFile("../" &ModelEname&"/Config.xml") Then
		  if RS("ChannelStatus")=1 then %>
        	<li id="chover" class="chover" onmouseover="$('#sub<%=rs("ChannelID")%>').show()" onmouseout="$('#sub<%=rs("channelid")%>').hide();">
			<%
            ...
            %>
			<br/><img src="../images/check.png" /><div class="CPlus" id="sub<%=rs("ChannelID")%>" style="display:none"><a  href="javascript:void(0)" onclick="delPlusl(this,<%=RS("ChannelID")%>)">关闭</a> 
            <%
        ...
        <%else%>
        	<li onmouseover="$('#sub<%=rs("ChannelID")%>').show()" onmouseout="$('#sub<%=rs("channelid")%>').hide();"><%=RS("ChannelName")%><br><img src="../images/check1.png" /><div id="sub<%=rs("ChannelID")%>" style="display:none"><a  href="javascript:void(0)" onclick="addPlusl(this,<%=RS("ChannelID")%>)">开启</a></div></li>
        <%end if
	    Else
		...
		For Each FsoItem In SubFolderObj
		   if KS.CheckFile("../plus/"&FsoItem.name&"/Config.xml") then
			   ...
			   if FieldXML.readystate=4 and FieldXML.parseError.errorCode=0 Then 
		        ...
		        If AppStatus="0" Then
				 %>
					<li id="chover" class="chover" onmouseover="$('#sub<%=AppEname%>').show()" onmouseout="$('#sub<%=AppEname%>').hide();">
					<%=AppName%>
					 <br><img src="../images/check1.png" /><div id="sub<%=AppEname%>" style="display:none"><a  href="javascript:void(0)" onclick="addPluslapp(this,'<%=AppEname%>')">开启</a></div>
					</li>
				<%
			    Else
				 %>
					<li id="chover" class="chover" onmouseover="$('#sub<%=AppEname%>').show()" onmouseout="$('#sub<%=AppEname%>').hide();">
					<%=AppName%>
					 <br><img src="../images/check.png" /><div class="CPlus" id="sub<%=AppEname%>" style="display:none"><a  href="javascript:void(0)" onclick="delPluslapp(this,'<%=AppEname%>')">关闭</a></div>
					</li>
				<%
				End If
	...
	'	On Error Resume Next
	   If KS.G("Action")="Edit" Then
		...
		" <tr><td height=5></td></tr>"&_
		"</table>" & _
		"<div class=tab-page id=modelpanel>"& _
...
%>
		...
		<dd><div><strong>模型名称：</strong></div><input class="txt" name="ChannelName" type="text" <%If KS.G("Action")<>"Edit" Then Response.Write " onkeyup='GetTable(this.value)'"%> value="<%=ChannelName%>" size="50">
        <span>如：文章系统，图片系统等。</span>
		</dd>
		<dd><div><strong>模型目录：</strong></div><input class="txt" name="ModelEname" type="text"<%If KS.G("Action")="Edit" Then Response.Write " Disabled"%> value="<%=ModelEname%>" size="50"> <span class="tips">只能用字母和数字的组合，且不能修改</span>
		</dd>

		<dd><div><strong>数据表名称：</strong></div><%If KS.G("Action")="Add" Then Response.Write " KS_U_" %><input name="ChannelTable" id='ChannelTable' type="text" value="<%=ChannelTable%>" class="txt" size="50"<%If KS.G("Action")="Edit" Then Response.Write " Disabled"%>><span>说明：创建数据表后无法修改，并且用户创建的数据表以"KS_U_"开头</span> 
		</dd> 
		...
		<dd><div><strong>项目名称：</strong></div><input class="txt" name="ItemName" id="ItemName" type="text" value="<%=ItemName%>" size="50"> <span class="tips">*如：文章、图片、软件等项</span></dd>
		<dd><div><strong>项目单位：</strong></div><input name="ItemUnit" type="text" value="<%=ItemUnit%>" class="txt" size="50"> <span class="tips">*如：篇、个、本等</span></dd>
		<dd><div><strong>栏目名称：</strong></div><input name="FolderName" type="text" value="<%=ThumbnailsConfig(26)%>" class="txt" size="50"> <span class="tips">*如：分类，品牌等</span></dd>
		
		<dd><div><strong>模型描述：</strong></div><textarea name="Descript" rows=4 cols=80><%=Descript%></textarea></dd>
		</dl>
		</div>
	...
	<div class="tips">Tips:积分奖励的规则请按成绩从高到低的顺序设置,删除选项请将成绩项留空即可。</div>
		...
		.Write "<dl class=""dtable""><dd><div>本模型运行模式：</div>"
		...
		.Write ">发布首页+栏目页+内容页"
		.Write "    </dd>"	
		.Write "    <dd><div>自动生成列表分页数：</div><input class='txt' style='text-align:center' type='text' value='" & FsoListNum & "' name='FsoListNum' size='10' /><span>这里设置生成栏目列表分页时自动生成的分页数，如果你的网站数据量较大，建议输入一个较小的数字，小数据量的网站可以不用限制，直接设置为0</span></dd>"	
	    .Write "<dd><div>生成的总目录：</div>"      
		.Write "<input class='txt' name='FsoFolder' type='text' value='" & FsoFolder & "' size='30'/><span class='tips'>*用于生成静态html存放的目录，只能以字母和数字的组合,必须以""/""结束</span>"
		.Write "</dd>"
		.Write "<dd><div>生成的栏目页规则：</div>"   
		...
		.Write "<dd><div><strong>生成的列表页的前缀字符：</strong></div><input class='txt' name='FsoClassPreTag' type='text' value='" & FsoClassPreTag & "' size='30'> <span class='tips'>*如list,show等</span><br/><span class='tips'>可用标签：<br/>{$ClassEname}-本栏目英文名<br/>{$ClassID}-本栏目小ID<br/> {$BigClassID}-本栏目大ID<br/>{$TopClassEname}-一级栏目英文名 "
		.Write "</dd>"


		.Write "<dd><div>生成的内容页目录规则：</div>"      
		.Write " <input class='txt' name='FsoContentRule' type='text' value='" & FsoContentRule & "' size='30'>&nbsp;"
	...
	End If
		...
		.Write "    <dd><div>允许上传的最大文件大小：</div><input name=""UpfilesSize"" class='txt' onBlur=""CheckNumber(this,'允许上传最大文件大小');"" type=""text"" id=""UpfilesSize"" value=""" & UpfilesSize & """ size=""10"">"
		...
		.Write "          <td><input class='txt' name=""AllowUpPhotoType"" type=""text"" id=""AllowUpPhotoType"" value=""" & AllowUpPhotoType & """ size=""30""></td>"
		.Write "        </tr>"
		.Write "        <tr>"
		...
		.Write "    <input class='txt' name=""ModelShortName"" type=""text"" id=""ModelShortName"" value=""" & ModelShortName & """ size=""50""> <span class='tips'>如：文章，新闻，房源等，建议取两个汉字名称</span>"
		.Write "    </dd>"
		...
		.Write "    <input class='txt' name=""ModelIco"" type=""text"" id=""ModelIco"" value=""" & ModelIco & """ size=""50""> <input class=""button""  type='button' name='Submit' value='选择图片...' onClick=""OpenThenSetValue('Include/SelectPic.asp?ChannelID=" & ChannelID &"&CurrPath=" & CurrPath & "',550,290,window,$('#ModelIco')[0]);""> <span class='tips'>如：/user/images/ico1.gif</span>"
		.Write "    </dd>"
		
		.Write "    <dd><div>新注册会员：</div>"
		.Write "    <input class='txt' name=""PubTimeLimit"" type=""text"" style=""text-align:center"" id=""PubTimeLimit"" value=""" & PubTimeLimit & """ size=""6"">分钟后才可以在此模型投稿"
		.Write "    </dd>"
		
		.Write "    <dd><div>会员投稿增加：</div> 资金<input class='txt' style='text-align:center' name=""UserAddMoney"" type=""text"" id=""UserAddMoney"" value=""" & UserAddMoney & """ size=""6"">元  点券<input class='txt' style='text-align:center' name=""UserAddPoint"" type=""text"" id=""UserAddPoint"" value=""" & UserAddPoint & """ size=""6"">点  积分<input class='txt'  name=""UserAddScore"" type=""text"" id=""UserAddScore"" value=""" & UserAddScore & """ style='text-align:center' size=""6"">分<span>为0时不增加,可设置成负数,表示投稿要消费</span>"
		.Write "    </dd>"
	...
	End If	 
		...
		.Write "     <div>下载本模型附件费用：</div><input class='txt' type=""text"" style=""text-align:center"" size=8 name=""AnnexPoint"" value=""" & AnnexPoint & """> 24小时内下载不重复扣费,不限制请输入0"
		.Write "    </dd>"
		...
		.Write "     <input class='txt' type=""text"" style=""text-align:center"" size=8 name=""LatestNewDay"" value=""" & LatestNewDay & """>天内添加的信息标志为最新信息"
		.Write "    </dd>"
		.Write "    <dd><div>生成的文档内容里的图片是否格式化：</div>"
        .Write "<input type='radio' name='formatcontentimg' value='0'"
		
		 if ThumbnailsConfig(27)="0" then .Write " checked"
		.Write ">不启用"
		...
		.Write "    <dd><div>后台管理设置：</div>后台每页显示：</strong> <input class='txt' type=""text"" style='text-align:center' size='4' name=""MaxPerPage"" value=""" & MaxPerPage & """>条信息"
		.Write "<br/>显示按栏目筛选："
		...
		.Write "      <td height=""30""> <input style=""text-align:center"" class='txt' name=""CommentLen"" type=""text"" value=""" & CommentLen & """ size=""6"">个字,同一个IP<input class='txt' name=""CommentTime"" type=""text"" style=""text-align:center"" value=""" & ThumbnailsConfig(4) & """ size=""6"">小时内对同一篇文档只能发表一次。不限制请输入""0""</td>"
		.Write "    </tr>"		
		.Write "    <tr valign=""middle"">"
		.Write "      <td height=""30"" align=""right"">内容页每页显示评论条数：</td>"
		.Write "      <td height=""30""> <input style=""text-align:center"" class='txt' name=""CommentPerPage1"" type=""text"" value=""" & ThumbnailsConfig(22) & """ size=""6"">条,更多评论页面每页显示<input class='txt' name=""CommentPerPage2"" type=""text"" style=""text-align:center"" value=""" & ThumbnailsConfig(23) & """ size=""6"">条</td>"
		.Write "    </tr>"		
		.Write "    <tr valign=""middle"">"
		.Write "      <td height=""30"" align=""right"">评论页模板：</td>"
		.Write "      <td height=""30""><input class='txt' name=""CommentTemplate"" id=""CommentTemplate"" type=""text"" value=""" & CommentTemplate & """ size=""50"">&nbsp;" & KSCls.Get_KS_T_C("$('#CommentTemplate')[0]") & "</td>"
		.Write "    </tr>"			
		.Write "      </table>"
		.Write "    </dd>"	
		If KS.WSetting(0)="1" Then
		.Write "    <dd style='display:none'>"
		.Write "      <div>3G搜索页模板：</div><input class='txt' name=""WapSearchTemplate"" id=""WapSearchTemplate"" type=""text"" value=""" & WapSearchTemplate & """ size=""50"">&nbsp;" & KSCls.Get_KS_T_C("$('#WapSearchTemplate')[0]")
		.Write "    </dd>"
		End If
		.Write "  </dl>"
		.Write "</div>"
