...
Class Admin_Field
		...
		.Write "<li class='parent' onclick=""FieldGroup();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/addfolder.gif' border='0' align='absmiddle'>字段分组</span></li>"
		End If
		...
		.Write "<li class='parent' onclick=""$('#action').val('setshowonform');$('#v').val('1');$('#myform').submit();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/set.gif' border='0' align='absmiddle'>设置后台显示</span></li>"
		.Write "<li class='parent' onclick=""$('#action').val('setshowonform');$('#v').val('0');$('#myform').submit();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/set.gif' border='0' align='absmiddle'>设置后台不显示</span></li>"
		.Write "<li class='parent' onclick=""$('#action').val('setshowonuserform');$('#v').val('1');$('#myform').submit();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/set.gif' border='0' align='absmiddle'>设置前台显示</span></li>"
		.Write "<li class='parent' onclick=""$('#action').val('setshowonuserform');$('#v').val('0');$('#myform').submit();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/set.gif' border='0' align='absmiddle'>设置前台不显示</span></li>"
		End If
		...
		.Write "';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/back.gif' border='0' align='absmiddle'>回上一级</span></li>"
		.Write "</ul>"		
      ...
      <script>
	   function check(){
		...
		With Response
		If ChannelID<>9 and ChannelID<>101 Then
			.Write "<div class=""tabs_header"">" &vbcrlf
		  ...
		  If KS.G("GroupID")<>"" Then Param=Param & "  and a.groupid=" & KS.ChkClng(KS.G("GroupID"))
			 Set FieldRS = Server.CreateObject("ADODB.RecordSet")
				 ...
				 Else
							...
							Call showContent
			End If
		 .Write "<tr><td colspan='3' style=""height:50px"">&nbsp;&nbsp; <input type='submit' onclick=""$('#action').val('order');"" class='button' value='批量保存排序'> </td></form>"
		...
		.Write "<body leftmargin='0' topmargin='0' marginwidth='0' marginheight='0'>"
		 ...
		 If Optype = "Edit" Then
			 ...
			 End If
	  Else
	     FieldName="KS_":FieldType=1:MustFillTF=0:ShowOnForm=1:ShowOnUserForm=1:ShowOnClubForm=0:AllowFileExt="jpg|gif|png":MaxFileSize=1024:Width=200:Height=80:EditorType="Basic":ShowUnit=0:MaxLength=255:GroupID=KS.ChkClng(Request("GroupID"))
	  ...
	  End If
		...
		.Write "  <form  action='KS.Field.asp?Action=" & OpAction &"' method='post' name='FieldForm' id='FieldForm'>"
		.Write "<input type='hidden' value='" & ChannelID & "' name='ChannelID'>"

        .Write "<dl class=""dtable"">" &vbcrlf
		
		.Write "    <dd><div>所属分组：</div>"
    ...
    If FieldType="0" Then  '系统内置字段
		.Write "   <dd><div>字段名称：</div>"
		.Write "    <input class='textbox' name='FieldName' type='text' readonly id='FieldName' value='" & FieldName & "' size='50'> </dd>" &vbcrlf
		.Write "    <dd><div>字段别名：</div><input name='Title' style=""width:350px"" type='text' id='Title' size='30' class='textbox' value='" & Title & "'> *<span>便于在管理项目中显示请为字段取别名</div></dd>"
        .Write "    <dd><div>字段类型：</div>"
        ...
        .Write "    <dd><div>选择要绑定的其它模型的分类：</div>"
	...
	Else
		.Write "   <dd><div>字段名称：</div> <input name='FieldName'  type='text' id='FieldName' value='" & FieldName & "' size='50'"
		If Optype = "Edit" Then .Write " readonly"
		.Write " class='textbox'> * <span>为了和系统字段区分，必须以""KS_""开头,在模板中可以通过""{$KS_字段名称}""进行调用</span>"
		.Write "    </dd>"
		.Write "    <dd><div>字段别名：</div>"
		.Write "     <input name='Title' type='text' id='Title' size='50' class='textbox' value='" & Title & "'> *<span>便于在管理项目中显示</span>"
		.Write "    </dd>"
		.Write "    <dd><div>附加提示：</div><textarea name='Tips'  id='Tips' class='textbox' style='width:340px;height:30px'>" & Tips & "</textarea><span class=""block"">在输入框旁边的提示信息,可以加入一些javascript事件</span>"
		.Write "    </dd>"
		.Write "    <dd><div>字段类型：</div>"
		If Optype = "Edit" Then
		.Write "     <input type='hidden' value=" & FieldType & " name='FieldType'><select style=""width:350px;"" name=""FieldType"" disabled>"
		else
		.Write "    <select name=""FieldType"" style=""width:350px;"" onchange=""Setdisplay(this.value)"">"
		end if
		...
		.Write "    <dd id=""ldArea"" style='display:none'><div>所属父级字段：</div>"
		  Dim PRS
    ...
    End If		
		...
		'  If FieldRS("FieldType")=5 And Not IsDate(DefaultValue) And DefaultValue<>"" Then Call KS.AlertHistory("默认值格式不正确!", -1): Exit Sub
		 ...
		 Call KS.Alert("字段修改成功!", KS.Setting(3) & KS.Setting(89) & "/system/KS.Field.asp?ChannelID=" & ChannelID&"&Page=" & Page &"&GroupID=" & KS.S("GroupID"))
		 End With
		 End Sub
