...
Dim RsObj,i,Flag,HtmlFileDir,ClassDir,strClassDir,MyFolderName
...
<body>
  ...
  With KS
	...
	.echo "<li onclick='CreateClass(0);' class='parent'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/addfolder.gif' border='0' align='absmiddle'>添加"& MyFolderName&"</span></li>"
	.echo "<li onclick='DelClass();' class='parent'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/del.gif' border='0' align='absmiddle'>删除"& MyFolderName&"</span></li>"
	if session("class_from")="all" or (request("channelid")<>"" and KS.C_S(request("channelid"),7)<>0) Then
	.echo "<li onclick='CreateHtml();' class='parent'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/addfolder.gif' border='0' align='absmiddle'>发布"& MyFolderName&"</span></li>"
	end if
	if session("class_from")="all" Then
	.echo "<li onclick='UniteClass();' class='parent'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/unite.gif' border='0' align='absmiddle'>"& MyFolderName&"合并</span></li>"
	.echo "<li onclick='OrderOne();' class='parent'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/s.gif' border='0' align='absmiddle'>一级"& MyFolderName&"排序</span></li>"
	.echo "<li onclick='OrderN();' class='parent'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/s.gif' border='0' align='absmiddle'>N级"& MyFolderName&"排序</span></li>"
	.echo "<li class='parent' onclick='MoveClassInfo();'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/move.gif' border='0' align='absmiddle'>"& MyFolderName&"文档移动</span></li>"
	...
	.echo "<div class=""quicktz""><a href='?a=extall&from=" & request("from") & "&channelid=" & request("channelid") &"'><img border='0' src='../images/ico/s.gif' align=""absmiddle"">全部展开</a></div>"
	.echo "</ul>"
...
Sub MainPage()
   With KS
	.echo "<form name='myform' id='myform' action='KS.Class.asp' method='post'>"
	if session("class_modelid")="" or request("channelid")="" then
    .echo "<div style='margin-top:5px'> &nbsp;"
	.echo "<select name='sc' onchange=""location.href='?from=all&a=" & request("a") & "&mid='+this.value;"">"
	...
	SQLstr = "select a.ID,a.FolderName,a.FolderOrder,a.ClassType,a.ChannelID,a.tj,a.tn,a.adminpurview,a.ts,a.child from KS_Class a inner join ks_channel b on a.channelid=b.channelid " & Param & " and (b.channelstatus=1 or a.channelid=5) Order BY root,folderorder"
    
	Dim RS:Set Rs = Server.CreateObject("adodb.recordset")
	Rs.Open SQLstr, Conn, 1, 1
	If RS.Eof And RS.Bof Then
	.echo "<tr class='list' onmouseout=""this.className='list'"" onmouseover=""this.className='listmouseover'""><td class='splittd' height='50' style='text-align:center' colspan=3>还没有添加"& MyFolderName&"！</td></tr>"
	Else
	       ...
	       Set ClassXML=KS.ArrayToXML(RS.GetRows(MaxPerPage),RS,"row","xmlroot")
			...
			For Each Node In ClassXML.DocumentElement.SelectNodes("row")
				...
				If KS.C("SuperTF")=1 or KS.FoundInArr(Node.SelectSingleNode("@adminpurview").text,KS.C("GroupID"),",") or Instr(KS.C("ModelPower"),KS.C_S(Node.SelectSingleNode("@channelid").text,10)&"1")>0 Then 
					...
					.echo "</tr>" & vbNewLine
					.echo "<tr><td id='sub" & ID &"' colspan=3>"
					.echo "</td></tr>"
			   End If
			Next
			End If
	  ...
	  For Each Node In Application(ks.SiteSN&"_class").documentelement.selectnodes("class[@ks13='"&parentid&"'" &p&"]")
	   ...
	   ClassType=Node.SelectSingleNode("@ks14").text
		...
		If (KS.C("SuperTF")=1 or KS.FoundInArr(Node.SelectSingleNode("@ks16").text,KS.C("GroupID"),",")) and (KS.C_S(Node.SelectSingleNode("@ks12").text,21)=1 or Node.SelectSingleNode("@ks12").text=5) or Instr(KS.C("ModelPower"),KS.C_S(Node.SelectSingleNode("@ks12").text,10)&"1")>0 Then 
	     ...
	     .echo " <tr class='list' onmouseout=""this.className='list'"" onmouseover=""this.className='listmouseover'"">"
		 .echo " <td style=""width:40px;padding-left:10px;text-align:left""><input type='checkbox' name='id' id='c" & id & "' value='" & id & "'></td>" & vbNewLine
		 .echo " <td style=""width:100px;padding-left:2px;text-align:left"">" & id & " </td>" & vbNewLine
