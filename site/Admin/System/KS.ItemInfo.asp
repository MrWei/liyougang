	...
	Public Sub Kesion()
		...
		Select Case Action
		 ...
		 Case "Special"
		  ...
		  End If
		 Case "SetAttribute"
			If Not KS.ReturnPowerResult(ChannelID, "M" & ChannelID & "10005") Then 
				 Call KS.ReturnErr(1, "")
			Else
		         Call SetAttribute()
			End If
	 ...
	 Sub CreateHtml()
	   ...
	   if ids="" then exit Sub
				...
				Response.Write "<table align='center' width='95%' height='200' class='ctable' cellpadding='1' cellspacing='1'><tr class='sort'><td  height='36' colspan=2>系统发布提示信息</td></tr> <tr class='tdbg'><td valign='top'>"
	   ...
	   If KS.C_S(ChannelID,7)=1 and KS.C_S(ChannelID,9)<>1 Then '栏目生成静态
		...
		End If   
			 ...
			 KS.Die "</td></tr>	  <tr><td  class='tdbg' height='25' align='center' colspan=2><input type='button' value=' 关 闭 ' onclick=""top.box.close();"" class='button'/></td>	  </tr>	</table> "
	 ...
	 Sub ItemInfoMain()
		...
		'	End If
		 ...
		 .echo "<li class='parent' onclick='VerificAll()'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/as.gif' border='0' align='absmiddle'>"
		 if KS.C("Role")="2" Then
		 .echo "批量初审"
		...
		.echo "<li class='parent' onclick='VerifyInfo();'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../images/ico/as.gif' border='0' align='absmiddle'>审核" & KS.C_S(ChannelID,3) & "</span></li>"
		if EnableRecycle<>1 then
        ...
        End If
			.echo "<div class='quicktz'>"
			  ...
			  .echo ("[共有 <font color=red>" & Conn.Execute("select count(id) from " & KS.C_S(ChannelID,2) & " where verific=1")(0) & "</font> " & KS.C_S(ChannelID,4) & " 回收站 <font color=blue>" &Conn.Execute("select count(id) from " & KS.C_S(ChannelID,2) & " where verific=1 and deltf=1")(0)  &"</font> "& KS.C_S(ChannelID,4) & "]")
			  end if
		 ...
		 .echo ("&nbsp;<img src='../Images/ico/search.gif' align='absmiddle'> 筛选结果: ")
	    ...
	    End If
		      ...
		      .echo ("<table style='clear:both;background:#E5ECF2;margin-top:5px;' height='35' border='0' width='100%' align='center'>")
			  .echo ("<tr><td>")
			  
			 
			  
			  .echo ("<img src='../images/ico/search.gif' align='absmiddle'> <strong>筛选：</strong>")
		 ...
		 .echo "</select>"
			  ...
			  .echo ("&nbsp;条件 <select name='searchtype'>")
			  If ChannelID=5 Then 
...
<div class="tabs_header">
    ...
    <li<%if KS.S("ShowType")="1" then response.write " class='active'"%>><a href="KS.ItemInfo.asp?showType=1&<%=KS.QueryParam("showtype")%>"><span>待审核的<%=KS.C_S(ChannelID,3)%></span></a></li>
    <%if VerifyJB=1 Then%>
    <li<%if KS.S("ShowType")="2" then response.write " class='active'"%>><a href="KS.ItemInfo.asp?showType=2&<%=KS.QueryParam("showtype")%>"><span>终审过的<%=KS.C_S(ChannelID,3)%></span></a></li>
    <li<%if KS.S("ShowType")="6" then response.write " class='active'"%>><a href="KS.ItemInfo.asp?showType=6&<%=KS.QueryParam("showtype")%>"><span>初审过的<%=KS.C_S(ChannelID,3)%></span></a></li>
    <%else%>
    <li<%if KS.S("ShowType")="2" then response.write " class='active'"%>><a href="KS.ItemInfo.asp?showType=2&<%=KS.QueryParam("showtype")%>"><span>已审核的<%=KS.C_S(ChannelID,3)%></span></a></li>
    <%end if%>
    <li<%if KS.S("ShowType")="3" then response.write " class='active'"%>><a href="KS.ItemInfo.asp?showType=3&<%=KS.QueryParam("showtype")%>"><span>草稿的<%=KS.C_S(ChannelID,3)%></span></a></li>
    <li<%if KS.S("ShowType")="4" then response.write " class='active'"%>><a href="KS.ItemInfo.asp?showType=4&<%=KS.QueryParam("showtype")%>"><span>被退回的<%=KS.C_S(ChannelID,3)%></span></a></li>
	...
	<% end if%>
	    ...
	    End If 
		 ...
		 .echo "<a style='float:right;padding-right:5px;' href=""javascript:;"" onclick='SetCol()'><img border='0' align='absmiddle' src='../images/bg18.png' title='设置显示管理的列'/></a>"
		 end if
		 .echo " 操 作 </td></tr>"
	   ...
	   End If
		...
		Else
			If DataBaseType=1 Then
					...
					Cmd.CommandText="KS_GetPageRecord"
					Cmd.CommandType=4	
					CMD.Prepared = true 
	...
	If KS.S("showType")="-1" Then
	   ...
	   .echo "<br/><input type='submit' value='批量保存排序' class='button'/>"
	   .echo "</td></tr></table>"
	Else	
    ...
    If Not IsObject(IXml) Then Exit Sub
			...
			For Each INode In IXml.DocumentElement.SelectNodes("row")
					...
					End If
						...
						Else
						 ...
						 Dim TLen:TLen=KS.ChkClng(KS.ReadSetting(2)) : If TLen<=0 Then TLen=30
							...
							.echo ("<span style=""cursor:default""> <br/><a href='javascript:View("&ItemId&");'>"& KS.Gottopic(INode.SelectSingleNode("@title").text,TLen))  &"</a>" & AttrStr 
					...
					End If	 
							 ...
							 ElseIf KS.C("Role")<>"1" and (ComeFrom="Verify" or ShowType=1 or ShowType=6)  Then
							  ...
							  Else
								 If Cint(INode.SelectSingleNode("@verific").text) <>3  Then   '已审核或草稿文章不允许操作
								   .echo "  <a href=""#""  onclick=""$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ChannelID=" & ChannelID & "&ComeFrom=Verify&ButtonSymbol=AddInfo&OpStr=" & server.URLEncode(KS.C_S(ChannelID,3) & "管理 >> <font color=red>签收会员" & KS.C_S(ChannelID,3)) & "</font>';location.href='" & ItemManageUrl & "?ChannelID=" & ChannelID & "&Page=" & Page & "&Action=Verify&ID="&ItemId&"';"">"
								   if VerifyJB=1 then
							 ...
							 Else
							  ...
							  End If
							 .echo (" <a href='javascript:editd("&ItemId&");'>修改</a> | ")
							 if EnableRecycle<>1 then
							 .echo ("<a href=""javascript:;"" onclick=""Recely("& ItemId & ");"">删除</a>")
							 else
							 .echo ("<a href=""javascript:;"" onclick=""Delete("& ItemId & ");"">删除</a>")
							 end if
      ...
      Function GetStatus(verific)
	     Select Case Cint(verific)
			...
			Case 1
			if VerifyJB=1 then
			GetStatus = "<span style='color:blue'>终审通过</span>"
			else
			GetStatus = "<span style='color:blue'>已审核</span>"
			end if
      ...
      Sub ShowClassList(ChannelID,ID)
		...
		If Not RSC.Eof Then 
		 ...
		 If IsObject(XML) Then
		   If ID<>"0" Then
		    .echo "<a href='?ChannelID=" & ChannelID & "&ID=" & ParentID & "'><img src='../images/folder/Back.png' border=0 align='absmiddle'/>返回</a> <div class='clear'></div>"
		   End if
		   For Each Node In XML.DocumentElement.SelectNodes("row")
		    ...
		    .echo "<li><img src='../images/folder/" & Img & "' align='absmiddle'><a href='?ChannelID=" & ChannelID & "&ID=" & Node.SelectSingleNode("@id").text & "' title='" & Node.SelectSingleNode("@foldername").text & "'>" & .Gottopic(Node.SelectSingleNode("@foldername").text,8) 
			if showclass=2 then
		...
		Else
		  If err Then
		   .echo "<img src='../images/folder/AddFolder.gif' align='absmiddle'>请先<a href='#' onclick=""location.href='KS.Class.asp?Action=Add&ChannelID=" & ChannelID & "';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Go&OpStr=" & Server.URLEncode("栏目管理 >> <font color=red>添加栏目</font>") & "';"">添加栏目</a>"
		  Else
		   .echo "<a href='?ChannelID=" & ChannelID & "&ID=" & ParentID & "'><img src='../images/folder/Back.png' border=0 align='absmiddle'></a> <a href='#' onclick='CreateNews()'><font color=#4169E1><strong>添加" & KS.C_S(Channelid,3) & "</strong></font></a>"
		   End If
		End If
		 .echo "</div>"
		...
		Sub SetAttribute()
		 ...
		 %>
			...
			<form name="myform" action="?Action=SetAttribute" method="post" target="_hiddenframe">
			  ...
			  <input type="hidden" value="Add" Name="Flag">
			<table width="99%" style="margin-top:10px" border="0" align="center"  cellspacing="1" class='ctable'>
