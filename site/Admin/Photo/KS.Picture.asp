...
Class Admin_Picture
		...
		Private FolderID, TemplateID,WapTemplateID,TN, TI,TJ,Action,OTid,OID,Province,City
  ...
  For Each TypeNode In Application(KS.SiteSN & "_FieldGroupXml").DocumentElement.SelectNodes("row[@channelid=" & ChannelID &"]")
	...
	Else
	     ...
	     Select Case lcase(FNode.SelectSingleNode("@fieldname").text)
	       case "title"
				.Write " <dd><div>" & XTitle & ":</div><input name='title' type='text' id='title'  class='rule' value='" & Title & "' size='80'/>"
				.Write "  "
				...
				.Write "   </dd>" &vbcrlf
		  case "tid"
		        .Write " <dd><div>" & Replace(XTitle,"栏目",KS.GetClassName(ChannelID)) & ":</div><input type='hidden' name='OldClassID' value='"& FolderID & "'>"
		...
		case "origin"
		 ...
		 case "area"
				...
				.write "</script>"
				.Write "   <div>" & XTitle & ":</div><script src=""../../plus/area.asp"" type=""text/javascript""></script>  <font color='#999999'>不限地区，请不要选择。</font></dd>" &vbcrlf
				.Write "<script type='text/javascript'>"
				if Province<>"" then
		  ...
		  case "photourl"
				...
				.Write "     </dd>" &vbcrlf
				.Write "   <dd>"
				.Write "     <iframe id='UpPhotoFrame' name='UpPhotoFrame' src='../System/KS.UpFileForm.asp?UPType=Pic&ChannelID=" & ChannelID &"' frameborder=0 scrolling=no width='100%' height='40'></iframe>"
				.Write "    </dd>" &vbcrlf
		  case "content"
				...
				%>
                <label<%if KS.TBSetting(5)="0" then response.write " style='display:none'"%>><input type="checkbox" name="AddWaterFlag" value="1" onClick="SetAddWater(this)" checked="checked"/>图片添加水印</label>)
				</font></div>
				<style type="text/css">
				...
				#thumbnails div.thumbshow{text-align:center;margin:2px;padding:2px;width:162px;height:200px;border: dashed 1px #B8B808; background:#FFFFF6;float:left}
				...
				<script type="text/javascript">
						...
						function addImage(bigsrc,smallsrc,text,sorn) {
							...
							newImgDiv.innerHTML += '<div style="margin-top:5px;text-align:left">'+delstr+' <b>注释：</b><input type="hidden" class="pics" id="pic'+pid+'" value="'+bigsrc+'|'+smallsrc+'"/><input type="text" class="textbox" name="picinfo'+pid+'" value="'+text+'" style="width:155px;" /> <span><a  title="左移动排序" href="javascript:;" onclick="pic_move(this,1);">←左移动</a>&nbsp;&nbsp;&nbsp;<a title="右移动排序" href="javascript:;" onclick="pic_move(this,2);">右移动→</a></span></div>';
							pid++;
		  ...
		  case "picturecontent"
				...
				.Write "  <div>" & XTitle & ":</div>"
				 .Write EchoEditor("Content",PictureContent,"Basic","96%","180px")
...
END IF
	...
	If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='attroption']/showonform").text="1" Then	 
		 ...
		 If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='adddate']/showonform").text="1" Then
			...
			End If
			.Write "                  <b><a href='#' onclick=""WdatePicker({el:$('input[name=AddDate]').get(0),dateFmt:'yyyy-MM-dd HH:mm:ss'});return false;""><img src='../Images/date.gif'  border='0' align='absmiddle' title='选择日期'></a>日期格式：年-月-日 时：分：秒"
			.Write "               </dd>"
	    End If
	  ...
	  If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='template']/showonform").text="1" Then
			...
			.Write "<div id='templatearea' style='font-weight:normal;display:none'>"
			If KS.WSetting(0)="1" Then .Write "WEB模板"
			...
			.Write "<div id='templatearea' style='font-weight:normal;'>"
			If KS.WSetting(0)="1" Then .Write "WEB模板 "
	  ...
	  If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='fname']/showonform").text="1" Then
			.Write "             <dd><div>" & FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='fname']/title").text & ":</div>"
			...
			.Write "<div id='filearea' style='font-weight:normal;display:none'><input name='FileName' type='text' id='FileName'   value='" & FileName  & "' size='25' class='textbox'> <font color=red>可带路径,如 help.html,news/news_1.shtml等</font></div>"
			End IF
			 .Write "                  </dd>"
	 End If
