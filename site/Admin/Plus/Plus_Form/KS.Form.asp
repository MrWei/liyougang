...
Class Admin_Form
		...
		Public Sub Kesion()
		  ...
		  End If
		   if  KS.G("Action")<>"print_bm" then
			...
			.Write "<li class='parent' onclick=""location.href='KS.Form.asp?action=Add';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Go&OpStr=自定义表单 >> <font color=red>添加表单</font>';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../../images/ico/a.gif' border='0' align='absmiddle'>添加表单</span></li>"
		...
		Sub Main()
		  ...
		  Do While Not RS.Eof 
			...
			.Write "<br/><strong>记录管理:</strong> <a href=""javascript:ShowResult(" & rs("id") &",0);"">横排查看</a>｜<a href=""javascript:ShowResult(" & rs("id") &",1);"">竖排查看</a> ｜<a href=""javascript:AddRecord(" & rs("ID") & ");"">添加记录</a>｜<a href='?Action=Import&ID=" & RS("ID") & "'>批量导入</a></td></tr>"
			RS.MoveNext 
		  Loop
		  End If
		    .Write "</table>"
		...
		Sub Total()
		  ...
		  Do While Not RS.Eof
			...
			Else
			.Write "<td width='50' class='splittd'></td><td width='140'  class='splittd'><img src='../../images/37.gif'>&nbsp;<b>" & RS("FormName") & "</b></td><td  class='splittd'><input type='text' class='textbox' value='&lt;script type=&quot;text/javascript&quot; src=&quot;{#GetFullDomain}/plus/form/form.asp?id=" & rs("id") & "&m={$ChannelID}&d={$InfoID}&quot;&gt;&lt;/script&gt;' name='s" & rs(0) & "' size=60></td><td><input class=""button"" onClick=""jm_cc('s" & rs(0) & "')"" type=""button"" value=""复制到剪贴板"" name=""button""></td><td></td>"
			End If
		...
		Dim TempStr,SqlStr, RS, InstallDir, FsoIndexFile, FsoIndexExt,i,UserIPTime,Email,SubmitUrl,SubmitTips,iponce
	...
	'	On Error Resume Next
	   If KS.G("Action")="Edit" Then
		...
		Else
		     iponce=0: Status=1:TimeLimit = 0:StartDate=Now():ExpiredDate=Now()+10:AllowGroupID="":useronce=0:onlyuser=0:shownum=1:UpLoadDir="form/":PostByStep=0:StepNum=1:ToUserEmail=0:AnonymousUpload=0:adminuserlist="":UserIPTime=0:MaxPerPage_s=20:SubmitUrl="":SubmitTips="恭喜,您的信息已提交成功!"
		End If
...
%>
		...
		<td height="30"><input name="StartDate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" id='StartDate' class="textbox" type="text" value="<%=StartDate%>" size="50"><br><font color=#ff0000>日期格式：0000-00-00 00:00:00</font></td>   
		</tr> 
		...
		<tr ID="EndDate" style="display:none" valign="middle" class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">      
			...
			<td height="30"> <input name="ExpiredDate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" id="ExpiredDate" class="textbox" type="text" value="<%=ExpiredDate%>" size="50"><br><font color=#ff0000>日期格式：0000-00-00 00:00:00</font></td> 
		</tr>
		...
		<tr valign="middle" class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">      
			<td width="160" height="30" class="clefttitle" align="right"><div><strong>允许直接在前台列表管理的用户名列表：</strong></div></td>      
			<td height="30"> 
			<textarea name="adminuserlist" style="width:370px;height:90px" id="adminuserlist" class="textbox"><%=adminuserlist%></textarea>
		<div class="tips">	如果允许普通管理员直接在前台删除，修改，答复表单的留言数据，在此输入允许管理表单留言数据的用户名，多个用户用英文逗号隔开，如admin,kesion等。
		</div>
		</td>
		</tr>
			<tr valign="middle" class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">      
	  ...
	  <body>
		<table width='100%' border='0' cellspacing='0' cellpadding='0'>
		  <tr>
			<td height='25' class='sort'>自定义表单[<%=FormName%>]模板管理</td>
		 </tr>
		...
		Sub AutoTemplate()
		 ...
		 Dim ShowNum,PostByStep,StepNum,K,Param,S,KK,Cipher
		 Dim SQL,N,O_Arr,O_Len,F_V,BrStr,O_Value,O_Text
		 Dim FormID:FormID=KS.ChkClng(KS.G("FormID"))
		 Dim dy:dy=KS.G("dy")
		 Dim RS:Set RS=Server.CreateObject("ADODB.RECORDSET")
		 RS.Open "Select top 1 ShowNum,PostByStep,StepNum,Cipher From KS_Form Where ID=" & FormID,conn,1,1
		 If Not RS.Eof Then
		  ShowNum=RS(0):PostByStep=RS(1):StepNum=RS(2):Cipher=RS(3)
		 End If
		 RS.Close
		 ...
		 For S=1 To StepNum
			 ...
			 Else
			 Response.Write "<iframe src=""about:blank"" name=""hidform" & FormID & """ style=""display:none""></iframe>" &vbcrlf
			 End If
			 ...
			 if  dy<>"ok" then
			    If PostByStep=1 Then
				 Response.Write "<form name=""myform"" action=""" & ks.setting(3) & "plus/form/form.asp"" onSubmit=""return(form_c());""   method=""post""> " &vbcrlf
				Else
				 Response.Write "<form name=""myform"" action=""" & ks.setting(3) & "plus/form/form.asp"" onSubmit=""return(form_c());""  target=""hidform" & FormID & """ method=""post""> " &vbcrlf
				End If
			 ...
			 If SQL(6,K)=1 and SQL(3,K)<>3 then
				 Response.Write "<span class=""formstrck"" id=""" &SQL(0,K)&"|%|"&SQL(1,K)&"|%|"&SQL(3,K)&""" style=""display:none""></span>"
			 end if		
	  ...
	  <body>
		...
		<div style="width:100%;padding-bottom:5px;margin-bottom:5px;overflow-x: auto; height:auto">
		 <%
 			set rs=server.createobject("adodb.recordset")
		...
		Sub Replay()
		 ...
		 <form action="KS.Form.asp?action=replaysave&formid=<%=formid%>&id=<%=id%>" method="post" name="myform" target="hiddenframe">
		  ...
		  <tr class="tdbg">
		   ...
		   <td><%
		   		 Response.Write "<script id=""content"" name=""content"" type=""text/plain"" style=""width:620px;height:200px;"">" &rs("note")&"</script>"
	             Response.Write "<script>setTimeout(""var editor = " & GetEditorTag() &".getEditor('content',{toolbars:[" & GetEditorToolBar("Basic") &"],wordCount:false,autoHeightEnabled:false,minFrameHeight:200 });"",10);</script>"
		   %>
		   </td>
		  </tr>
		  <tr class="tdbg">
