	   ...
	   Sub Step1()
		...
		dim bstr,BasicType,wherestr	
		 ...
		 With KS
			...
			<div id="option">
			<br style="clear:both"/><label><input type="checkbox" name="refname" value="1" checked="checked" />自动更新目标生成的HTML文件名,以防止文件名重复存在。 </label>当目标数据表为空时，不建议打勾。
			<br/><label><input type="checkbox" name="retemplate" value="1" checked="checked" />自动绑定模板为目标栏目的默认文档模板。 </label>
			</div>
			<%
			.echo "</td>"
			.echo "    </tr>"
