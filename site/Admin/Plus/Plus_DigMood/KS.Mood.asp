...
Class Admin_MoodCls
		...
		Public Sub Kesion()
		  With KS
		   ...
		   End If
			...
			.echo"<ul id='menu_top'>"
			.echo"<li class='parent' onclick=""location.href='KS.Mood.asp?action=Add&TypeFlag=" &TypeFlag&"';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Go&OpStr=" & ItemStr & " >> <font color=red>添加" & ItemStr  &"项目</font>';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../../images/ico/a.gif' border='0' align='absmiddle'>添加" & ItemStr  &"项目</span></li>"
		...
		Sub Main()
		  ...
		  Do While Not RS.Eof 
			...
			if typeflag=1 then
				.echo"<a href='#' onClick=""window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=点评管理 >> <font color=red>查看点评数据</font>&ButtonSymbol=Disabled'; location.href='KS.Comment.asp?projectid=" & rs("ID") & "';"">查看管理用户点评</a>｜"
			else
			.echo"<a href='#' onClick=""window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=子系统 >> <font color=red>查看详情</font>&ButtonSymbol=Disabled';location.href='KS.Mood.asp?typeflag=" & typeflag&"&MoodID=" & rs("ID") & "&action=Show';"">查看详情</a>｜"
			end if

			.echo"<a href='?typeflag=" & typeflag &"&action=Edit&ID=" & rs("ID") & "' onclick=""$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=GoSave&OpStr=子系统 >> <font color=red>" & ItemStr & "</font>';"">修改</a>｜"
		...
		Sub GetCode()
		  ...
		  Do While Not RS.Eof
			...
			.echo "</td><td></td><td></td>"
			Else
			.echo "<textarea style='width:500px;height:50px' name='s" & rs(0) & "'>&lt;script language=&quot;javascript&quot; type=&quot;text/javascript&quot; src=&quot;" & KS.Setting(2) & "/plus/digmood/mood.asp?id=" & rs("id") & "&c_id={$InfoID}&M_id={$ChannelID}&quot;&gt;&lt;/script&gt;</textarea>"
...
%>
		...
		<tr valign="middle" class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">      
			...
			<%
			 ...
			 .echo " 图片地址 <input type=""text"" class=""textbox"" name=""Pic" & I & """ value=""" & PicValue & """></span><br>"
			 Next
			 %>
