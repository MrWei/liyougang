...
Sub ShowMain()
	...
	Do While Not Rs.EOF
		...
		If Rs("depth") = 0 Then
			Response.Write "<a href=""javascript:addserver(" &Rs("downid") &");"">添加服务器</a>" & vbcrlf
		Else
		    If Rs("isLock") <> 1 Then
			...
			End If
		End If
		Response.Write " | <a href=""javascript:editserver(" & Rs("downid") & ");"">修改</a>" & vbcrlf
...
Sub sEdit()
	...
	Response.Write "</td>" & vbcrlf
	Response.Write " </tr></table>" & vbcrlf
	Response.Write " <dl class=""dtable""><dd><div>所属类别：</div>" & vbcrlf
	Response.Write " <select name=""servers"" id=""servers"" onchange='setdisabled(this.value)'>" & vbcrlf
	...
	Response.Write " <div>下载得分：</div>" & vbcrlf
	...
	Response.Write " <dd id='s2'>" & vbcrlf
	Response.Write " <div>得分间隔时间：</div>" & vbcrlf
	...
	Response.Write " <dd id='s2'>" & vbcrlf
	Response.Write " <div>服务器路径：</div>" & vbcrlf
	...
	Response.Write " <div>是否直接显示下载地址：</div>" & vbcrlf
	...
	Response.Write " <div>是否外部连接：</div>" & vbcrlf
