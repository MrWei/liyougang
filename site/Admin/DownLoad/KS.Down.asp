...
Class Admin_Down
		...
		Private FolderID, TemplateID,WapTemplateID,Action,FileName,oTid,Oid
	...
	sub folderlist(uploadDir)
	  ...
	  Set FileObj = FolderObj.Files
	   ...
	   if FolderObj.subfolders.count>0 then
		  ...
		  For Each FsoItem In FolderObj.subfolders
			ks.echo "<tr><td><img src='../images/folder/folder.gif' align='absmiddle' /><a href='KS.Down.asp?action=SelectFolder&channelid=" & channelid & "&path=" & uploadDir & fsoitem.name & "/'>" & FsoItem.Name & "</a></td><td><label><input type='checkbox' onclick=""top.frames['MainFrame'].returnFolder('" &uploadDir & fsoItem.Name & "/')"">选择</label></td></tr>"
		  Next
	   else
	     if ParentPath<>"" then
	...
	For Each FNode In FieldXML.DocumentElement.SelectNodes("fielditem[showonform=1&&fieldtype!=13&&@groupid=" & TypeNode.SelectSingleNode("@id").text & "]")
		...
		Else
	     ...
	     Select Case lcase(FNode.SelectSingleNode("@fieldname").text)
	       case "title"
				...
				.Write "  <input name='title' type='text'  class='rule' value='" & Title & "' size=50><font color='#FF0000'>*</font>"
		...
		case "uploadphoto"
			   If CBool(UpPowerFlag) = True Then
				.Write "  <dd><iframe id='UpPhotoFrame' name='UpPhotoFrame' src='../System/KS.UpFileForm.asp?ChannelID=" & ChannelID & "&UpType=Pic' frameborder=0 scrolling=no width='100%' height='35'></iframe>"
				.Write " </dd>"& vbcrlf
				End If
       case "keywords"
		...
		case "content"
				...
				.Write "  <div>" & XTitle & ":<font>(<input name='BeyondSavePic' type='checkbox' value='1'>自动下载简介里的图片)</font></div>"
				 ...
				 .Write EchoEditor("Content",DownContent,"Basic","96%","200px")
...
END IF
	...
	If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='attroption']/showonform").text="1" Then	
		...
		If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='rank']/showonform").text="1" Then
			.Write "              <dd>"
			.Write "                <div>" & FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='rank']/title").text & ":</strong><select name='rank'>"
	 ...
	 If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='template']/showonform").text="1" Then
			 .Write "             <dd>"
			 .Write "               <div>" & FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='template']/title").text & ":</div>"
			...
			.Write "<div id='templatearea' style='font-weight:normal;font-size:12px;display:none'>"
			...
			.Write "</div>"
			Else
			.Write "<div id='templatearea' style='font-weight:normal;font-size:12px;'>"
			If KS.WSetting(0)="1" Then .Write "WEB模板 "
	  ...
	  If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='fname']/showonform").text="1" Then
				...
				.Write "<div id='filearea' style='font-weight:normal;font-size:12px;display:none'><input name='FileName' type='text' id='FileName'   value='" & FileName  & "' size='25' class='textbox'> <font color=red>可带路径,如 help.html,news/news_1.shtml等</font></div>"
				End IF
				 .Write "                  </dd>"
		 End If
End If
