...
Sub movielist()
	...
	RS.Open "Select Top " & Num & " ID,Title,Tid,Fname,photourl From KS_Movie Where ID In(" & KS.R(KS.C("View" & ChannelID)) & ") order by id desc",conn,1,1
...
Sub ArticleList()
    str=""
	Set RS=Server.CreateObject("ADODB.RECORDSET")
	RS.Open "Select Top " & Num & " ID,Title,Tid,Fname From " & ks.c_s(channelid,2) &" Where ID In(" & KS.FilterIds(KS.C("View" & ChannelID)) & ") order by id desc",conn,1,1
	If Not RS.Eof Then SQL=RS.GetRows(-1):RS.Close:Set RS=Nothing
	If IsArray(SQL) Then  
	  For K=0 To Ubound(SQL,2)
	   Url=KS.GetItemURL(ChannelID,sql(2,k),sql(0,k),sql(3,k))
