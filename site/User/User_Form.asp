...
Class User_userform
       ...
       if ks.s("action")="delitem" then
		...
		RS.Open "Select FormName,PostByStep,TableName,Template,Templ_url,id From KS_Form where status=1 ",conn,1,1
		...
		while not rs.eof
			...
			%>
			<div style=" padding-top:5px;"><input name="" class="button" type="button" onclick="$('.titlename').find('td').show();$('.tdbg').find('td').show();" value="显示所有字段" />
			</div>
