		...
		<input type="hidden" value="dosave" name="flag"/>
		 <table  cellspacing="1" cellpadding="3"  width="98%" align="center" border="0" class="border">
              ...
              <tr class="tdbg">
                ...
                <td><%=formatnumber(ordertotalmoney,2,-1,-1)%> 元</td>
              </tr>
              <tr class="tdbg">
                <td  class="clefttitle" align="right">已 付 款：</td>
                <td><%=formatnumber(MoneyReceipt,2,-1,-1)%> 元</td>
              </tr>
              <tr class="tdbg">
         ...
         <tr class='tdbg' onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
                  <td height='30' nowrap="nowrap" class="splittd bgtitle" colspan="5">
					    &nbsp;订单编号：<a href="User_Order.asp?Action=ShowOrder&ID=<%=RS("ID")%>"><%=rs("orderid")%></a> 
						 <%
						 if rs("ordertype")="1" then  response.write "<font color=red><b><i>团</li></b></font>"
				         %>合计：<%=formatnumber(rs("NoUseCouponMoney"),2)%>元(含运费：<%=rs("Charge_Deliver")%>元)
						<br /> <strong>应付
...
End function
  ...
  <Input id=ID type=hidden value="<%=rs("id")%>" name="ID"> 
	...
	If RS("MoneyReceipt")<RS("MoneyTotal") Then
	OrderDetailStr=OrderDetailStr & " 尚欠￥<font color=red>" & formatnumber(RS("MoneyTotal")-RS("MoneyReceipt")+rs("UseCouponMoney"),2) &"</font>元"
	End If
